/* Este codigo ha sido generado por el modulo psexport 20180802-w32 de PSeInt.
Es posible que el codigo generado no sea completamente correcto. Si encuentra
errores por favor reportelos en el foro (http://pseint.sourceforge.net). */

// En java, el nombre de un archivo fuente debe coincidir con el nombre de la clase que contiene,
// por lo que este archivo debería llamarse "SALARIOS_PROBLEMA_PAGINA_73.java."

import java.io.*;

public class salarios_problema_pagina_73 {

	public static void main(String args[]) throws IOException {
		BufferedReader bufEntrada = new BufferedReader(new InputStreamReader(System.in));
		int v_ht;
		String v_ne;
		double v_neto;
		double v_sb;
		double v_tasas;
		double v_vh;
		// Enunciado: Calcular el salario bruto y el salario neto de un trabajador "por horas" conociendo el nombre, número de horas trabajadas, impuestos a pagar y salario neto.
		// Leer ne, ht y vh
		// Pedir cada valor por teclado
		// desarrollado por Jonathan López
		// version 1.0
		// fecha  24/02/23
		// declaracion de variables
		// inicializacion de variables
		v_ne = "";
		v_ht = 0;
		v_vh = 0.0;
		v_sb = 0.0;
		v_tasas = 0.0;
		v_neto = 0.0;
		// entrada de datos
		System.out.println("Por favor escriba el nombre del empleado");
		v_ne = bufEntrada.readLine();
		System.out.println("Por favor escriba la cantidad de horas trabajadas");
		v_ht = Integer.parseInt(bufEntrada.readLine());
		System.out.println("Por favor escriba el valor hora");
		v_vh = Double.parseDouble(bufEntrada.readLine());
		// Proceos
		v_sb = v_ht*v_vh;
		v_tasas = v_sb*0.25;
		v_neto = v_sb-v_tasas;
		// Salida
		System.out.println("el nomnbre del empleado es= "+v_ne);
		System.out.println("el salario bruto es= "+"$"+v_sb);
		System.out.println("los impuestos o tasas a pagar son="+"$"+v_tasas);
		System.out.println("el salario neto es= "+"$"+v_neto);
	}


}

