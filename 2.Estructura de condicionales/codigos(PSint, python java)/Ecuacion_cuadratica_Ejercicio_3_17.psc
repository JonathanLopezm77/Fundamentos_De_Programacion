Proceso Ecuacion_cuadratica_Ejercicio_3_17
	...
	si v_b^2-4*v_a*v_c>0 Entonces
		...
		v_x1=(-v_b+RC(v_b^2-4*v_a*v_c))/2*v_a//formula generaal con más
		Escribir "(", v_x1, ",0", ")";
		Escribir"Elsegundo corte con x es";
		v_x2=(-v_b-RC(v_b^2-4*v_a*v_c))/2*v_a//formula general con -
		Escribir "(", v_x2, ",0", ")";
	SiNo  
		Escribir "Las raices son imaginarias";
		Escribir -v_b/(2*v_a), "+",(RC(abs(v_b^2-4*v_a*v_c)))/2*v_a, "i"//formulas generales con numeros imaginarios
	...
	v_y=v_a*(0)^2+v_b*(0)+v_c;//x=0 para encontrar el corte con y
	Escribir "(", "0,", v_y, ")";
	//se halla el corte con y en cordendas x,y por eso 0,y
	Escribir "El eje de simetría es";
	v_ejes=-v_b/2*v_a;//parta de la formula general para sacar el eje de simetría
	Escribir v_ejes;
	Escribir "El vertice en y es";
	v_very= v_a*(v_ejes)^2+v_b*(v_ejes)+v_c;//formula para sacar el vertice en cordenadas v,y
	Escribir v_very;
